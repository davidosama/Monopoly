...
//        curPlayer.move(m);
    ...
    public Boolean isOwned(int CityNum) {
        if (((Property) Constants.board.allCities.get(CityNum)).owned == true) {
            return true;
        }
        return false;
    }
    ...
    public void BuyCity(int city, Player player) {
        ...
        ((Property) Constants.board.allCities.get(city)).owned = true;

        for (int i = 0; i < Player.playersList.size(); i++) {
            if (Player.playersList.get(i) == player) {
                Boolean n = Player.playersList.get(i).buy(city, ((Property) Constants.board.allCities.get(city)).price);
                ((Property) Constants.board.allCities.get(city)).owner = i;
                if (n) {
    ...
    public void PayRent(int city, Player player) {
        JOptionPane.showMessageDialog(null, "Unfortunately,This city is owned by Player " + Player.playersList.get(((Property) Constants.board.allCities.get(city)).owner).num + " so you will have to pay him a rent");
        for (int i = 0; i < Player.playersList.size(); i++) {
            if (Player.playersList.get(i) == player) {
                Player.playersList.get(i).deductMoney(((Property) Constants.board.allCities.get(city)).OverallRent);
                Player.playersList.get(((Property) Constants.board.allCities.get(city)).owner).addMoney(((Property) Constants.board.allCities.get(city)).OverallRent);
